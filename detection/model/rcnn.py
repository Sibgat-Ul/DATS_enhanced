...
def rcnn_dkd_loss(stu_predictions, tea_predictions, gt_classes, alpha, beta, temperature):
    ...
    loss_dkd = dkd_loss(stu_logits, tea_logits, gt_classes, alpha, beta, temperature, False)
    return {
        'loss_dkd': loss_dkd,
    }

def rcnn_kd_loss(stu_logits, tea_logits, stu_ce, temperature):
    loss_kd = 0.9 * kd_loss(stu_logits, tea_logits, temperature, False)
    loss_ce = 0.1 * stu_ce
    return {
        'loss_dkd': loss_kd + loss_ce,
    }
...
class DynamicTemperatureScheduler:
    def __init__(
        ...
        self.current_temperature = args.INIT_TEMPERATURE
...
class RCNNKD(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.temperature_scheduler = DynamicTemperatureScheduler(self.kd_args)
    ...
    def forward(self, batched_inputs: Tuple[Dict[str, torch.Tensor]]):
        ...
        if self.kd_args.type == "kd":

            stu_logits, stu_bbox_offsets = self.forward_pure_roi_head(self.roi_heads, features, sampled_proposals)
            tea_logits, tea_bbox_offsets = self.forward_pure_roi_head(self.teacher.roi_heads, t_features, sampled_proposals)
            gt_classes = [x.gt_classes for x in sampled_proposals]
            gt_classes = torch.cat(tuple(gt_classes), 0).reshape(-1)

            stu_ce = torch.nn.functional.cross_entropy(stu_logits, gt_classes)
            tea_ce = torch.nn.functional.cross_entropy(tea_logits, gt_classes)

            with torch.no_grad():
                ld = tea_ce.item() - stu_ce.item()
                self.temperature_scheduler.update_temperature()


            detector_losses.update(
                rcnn_kd_loss(
                    stu_logits,
                    tea_logits,
                    stu_ce,
